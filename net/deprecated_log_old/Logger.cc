...
thread_local uint64_t Logger::lastLogSecond_ = -1;
thread_local uint64_t Logger::lastLogMSecond_ = -1;
...
void Logger::Flush(enum LogLevel level)
{
    ...
    // inuse标志使得这里不需要锁了，减少竞争
    // Format: level info, length, log msg
    int logLevel = level;

    info->buffer_.Write(&logLevel, sizeof logLevel);
    info->buffer_.Write(&pos_, sizeof pos_);
    info->buffer_.Write(tmpBuffer_, pos_);
...
void LogManager::Start()
{
    ...
    ThreadPool::Instance().ExecuteTask(std::bind(&LogManager::Run, this));
}
