...
bool  OutputMemoryFile::Open(const char* file, bool bAppend)
{
    ...
    if (bAppend)
    {
        struct stat st;
        fstat(file_, &st);
        size_ = std::max<decltype(size_)>(kDefaultSize, st.st_size);
        offset_ = st.st_size;
    }
    else
    {
...
bool OutputMemoryFile::_MapWriteOnly()
{
    ...
    {
        //std::cerr << "_MapWriteOnly failed " << size_ << ", file_ " << kInvalidFile << std::endl;
        assert (false);
        return false;
    }
