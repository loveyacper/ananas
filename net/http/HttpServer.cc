...
void HttpServer::OnDisconnect(Connection* conn) {
...
HttpContext::HttpContext(llhttp_type type, std::shared_ptr<Connection> c, HttpServer* server)
    : parser_(type), conn_(c), server_(server) {
  parser_.SetRequestHandler(std::bind(&HttpContext::HandleRequest, this, std::placeholders::_1));
}

int HttpContext::Parse(Connection*, const char* data, int len) {
